...
namespace FinanceAnalyzer
{
    public partial class FormMain : Form
    {
        ...
        void FindVertex()
        {
            var vps = FindVertexes();

            foreach (var item in vps)
            {
                LogMgr.Logger.LogInfo("Date: " + item.VertexDate
                    + ", Type: " + item.VertType);
            }
        }

        ICollection<VertexPoint> FindVertexes()
        {
            History_.MaxDate = dateTimePickerEnd.Value;
            History_.MinDate = dateTimePickerStart.Value;
            ...
            List<VertexPoint> vps = new List<VertexPoint>();

            foreach (var item in res)
            {
                VertexPoint p = new VertexPoint(item.VertType, item.FindType, History_.FindDateTime(item.DateID));
                vps.Add(p);
            }
